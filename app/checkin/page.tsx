...
export default function Checkin() {
        ...
        callback: async (resp: any) => {
          ...
          try {
            const res = await fetch(APP_SCRIPT_URL, {
              ...
              body: JSON.stringify({ idToken, event: eventId, ua: navigator.userAgent }),
            });
            const data = await res.json();
            if (data.ok) {
              setStatus(
                `✅ Điểm danh thành công!\nEmail: ${data.email}${
                  data.mapped?.name ? ' (' + data.mapped.name + ')' : ''
                }`,
              );
            } else if (data.duplicate) {
              setStatus(`⚠️ Bạn đã điểm danh cho ${data.event} trước đó.`);
            } else {
              setStatus('❌ Lỗi: ' + (data.error || 'Không xác định'));
            }
          } catch {
  ...
  return (
    <main style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16,padding:24,textAlign:'center'}}>
      ...
      <p><b>Sự kiện:</b> {eventId}</p>
      <div id="gbtn"></div>
      <pre style={{background:'#f7f7f7',padding:16,borderRadius:8,width:'90%',whiteSpace:'pre-wrap'}}>{status}</pre>
    </main>
  );
}
